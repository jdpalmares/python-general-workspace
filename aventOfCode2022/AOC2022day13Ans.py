        ...
        if val_one == val_two:
            ...
            if type_one != type_two:
                if "]" in [val_one, val_two]:
                    return val_one == "]"
                to_edit = lst[next((i for i, x in enumerate(
                    [type_one, type_two]) if x == int))]
...
with open("packetpairinput.txt", "r") as file:
    data = file.read().replace("[", "[,").replace("]", ",]")
    data = [x.splitlines() for x in data.split('\n\n')]
    p1 = 0
    for e, packets in enumerate(data):
        for i, packet in enumerate(packets):
            data[e][i] = [int(x) if x.isdigit()
                          else x for x in packet.split(",") if x]
        if compare(data[e][0][:], data[e][1][:]):
            p1 += e + 1
    data = sum(data, []) + [(two_extra := ["[", "[", 2, "]", "]"]),
                            (six_extra := ["[", "[", 6, "]", "]"])]
    ...
    print("sorted_data value = ", sorted_data)
    print("day 13: ", p1, (sorted_data.index(two_extra) + 1)
          * (sorted_data.index(six_extra) + 1))
